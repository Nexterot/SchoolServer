...
// createTopicMessageRequest используется в CreateTopicMessageHandler
...
func (rest *RestAPI) CreateTopicMessageHandler(respwr http.ResponseWriter, req *http.Request) {
	rest.logger.Info("REST: CreateTopicMessageHandler called", "IP", req.RemoteAddr)
	...
	var rReq createTopicMessageRequest
	...
	err = remoteSession.CreateForumThemeMessage("1", rReq.Message, strconv.Itoa(rReq.ID))
	if err != nil {
	...
	rest.logger.Info("REST: Successfully created topic message", "IP", req.RemoteAddr)
