...
| `date`    | Дата. |
| `message`    | Сообщение. |
| `files`    | Ссылки (на наш сервер), по которым наш сервер будет стримить присоединенные файлы с сайта. |
...
| Ключ        | Значение                              |
| ----------- | ------------------------------------- |
| `days`  | Количество дней для которых нужно расписание. |
| `date`  | Датав формате "dd.mm.yyyy", начиная с которой нужно расписание на N дней . |
| `id`    | Если у нас аккаунт родителя, у которого несколько детей учится в школе, то ему на сайте в дневнике доступно между ними переключение. Это id выбранного ребенка. Если ребенок один или это аккаунт ученика, то `id` = "" |
