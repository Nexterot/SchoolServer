...
| `date`    | Дата. |
| `id`    | Id урока. |
| `status`    | `true` если задание новое. `false` если задание выполнено. `undefined`  если просмотренное. |
...
Ответ на запрос: Пока не придуман. HTML страничка с отчетом или пропарсенные данные в JSON

#### 2.4.4 Получение списка предметов для отчета "Отчет об успеваемости".
...
#### 2.4.5 Получение отчета "Отчет об успеваемости".
...
#### 2.4.6 Получение отчета "Отчет об успеваемости и посещаемости".
...
Излишне, так как достаточно лишь ID ребенка. Класс игнорируется.

#### 2.4.8 Получение отчета "Отчет о доступе к классному журналу".
...
#### 2.4.10 Получение отчета "Информационное письмо для родителей".
